...
#define GLOB_CONFIG malog::GlobalConfig::Get()
...
void LoggerImpl::init_data()
{
   ...
   {
      m_logging = malog::make_unique<AsyncLogging>(
...
{
   ctx->level = m_level;
   ctx->tid   = malog::ProcessInfo::GetTid();
   ctx->err   = errno;
   detail::LoggerImpl::GetInstance().DoConfigLog(
     m_config ? m_config : &GlobalConfig::Get(), ctx);
}

logger_helper malog::Check(bool cond, source_location const& location)
{
   if (!cond) { return logger_helper{location}; }
   return {};
}

void malog::CheckIfFatal(bool cond, source_location const& location,
                         const char* text)
{
