...
void MyRpcChannel::CallMethod(google::protobuf::MethodDescriptor const *method,
							  google::protobuf::RpcController *controller, google::protobuf::Message const *request,
							  google::protobuf::Message *response, google::protobuf::Closure *done) {
	google::protobuf::ServiceDescriptor const *sd = method->service();
	...
	// 读取配置文件rpcserver的信息（直接和某一个rpc服务端连接，并非分布式）
	// rpc调用方想调用service_name的method_name服务，需要查询zk上该服务所在的host信息

	// 向zookeeper请求rpc服务端的信息，做到分布式
	ZKClient zkCli;
	zkCli.Start();
	//  /UserServiceRpc/Login
	std::string method_path = "/" + service_name + "/" + method_name;
	// 例如本地环回 127.0.0.1:8000
